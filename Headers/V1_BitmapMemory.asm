...
PrintString:
    STMFD sp!, {r0-r12, lr}     ; Save all working registers and return address

PrintStringAgain:
    ldrb r0, [r1], #1           ; Load byte from [r1], increment r1
    cmp r0, #0                ; Check for terminator
    beq PrintStringDone         ; If 255, done
    
    ; Skip non-printable characters (optional, 32–126 printable ASCII)
    CMP r0, #32
    BLT PrintStringAgain
    CMP r0, #127
    BGE PrintStringAgain
    
    bl  PrintChar               ; Call character print
    b   PrintStringAgain        ; Loop

PrintStringDone:
	bl NewLine
    LDMFD sp!, {r0-r12, lr}     ; Restore all registers and return address
...
FillScreen:
	STMFD SP!,{R0-R12, LR}
		...
		;--- FILLS SCREEN WITH THE SELECTED RGB BITS ---
		mov r0, #0x06800000	;Top Screen
		mov r3, #0x06200000 ;Bottom screen
		mov r2, #256*192	;Pixels to fill
		
				;  ABBBBBGGGGGRRRRR	A=Alpha
		mov r1, #ColorScreen	;Color BG when cleared bits
		add r1,r1,#0x808A0000
	
Fill_loop:
		strh r1, [r0],#2	;Color Top Screen
		strh r1, [r3],#2	;Color Bottom Screen
		subs r2, r2, #1
		bne Fill_loop
...
cls:
		...
		mov r2, #256*192	;Pixels to fill
				...
				;  ABBBBBGGGGGRRRRR	A=Alpha
		mov r1, #0b1000000000000000	;Color BG when cleared
		add r1,r1,#0x808A0000
CLS_loop:
		strh r1, [r0],#2	;Clear Top Screen
		strh r1, [r3],#2	;Clear Bottom Screen
		subs r2, r2, #1
		bne CLS_loop
	MOV PC,LR
